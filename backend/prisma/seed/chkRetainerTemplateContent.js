// Shared source of truth for CHK Immigration Services' retainer agreement
// template content — used by both seedChkRetainerTemplate.js (first-time
// creation) and resetChkRetainerTemplate.js (restoring this original content
// over any edits made since, e.g. through the Settings template editor).
//
...
//
// RCIC identity fields use {{owner.*}} rather than {{consultant.*}} — the
// agency asked for this to always show the designated agency owner/signing
// authority (Settings > Workspace Profile), not whichever staff member
// happens to be assigned to a given case. Falls back to the assigned
// consultant automatically until AgencyProfilePanel's owner fields are
// filled in (see renderCorrespondenceHtml).
//
// Markup here is deliberately restricted to what the CaseDesk Writer's
// Tiptap schema actually supports (headings, paragraphs, lists, tables, one
// block image) — no wrapper <div>s or inline styles beyond text-align,
// which the editor's registered extensions are configured to preserve.
// Anything else gets silently stripped the moment a draft is opened and
// autosaved.

export const AGENCY_NAME_MATCH = "CHK Immigration";
export const TEMPLATE_TITLE = "CHK Immigration Retainer Agreement";
export const TEMPLATE_SLUG = "chk-immigration-retainer-agreement";
export const TEMPLATE_DESCRIPTION =
  "CHK Immigration's own retainer agreement, extracted from the agency's real (signed) template and genericized with merge fields — now the general-purpose system default.";
export const TEMPLATE_CASE_TAGS = ["general"];
// Marks this as THE retainer template for Billing-tab resolution
// (caseBillingRetainerController's resolveRetainerTemplate) — distinct from
// isSystemDefault, which several other Agreement templates also carry.
export const TEMPLATE_IS_RETAINER = true;

export const TEMPLATE_CONTENT_HTML = `
<img src="{{agency.logoUrl}}" alt="{{agency.name}}" style="max-height:80px;max-width:280px;display:block;margin:0 auto 6px;" />
<h2 style="text-align:center;">{{agency.name}}</h2>

<p style="text-align:right;">RCIC Membership Number: {{owner.licenseNumber}}<br/>Client File Number: {{case.fileNumber}}</p>

<h1 style="text-align:center;">RETAINER AGREEMENT</h1>

<p>This Retainer Agreement is made on {{current.longDate}}, between Regulated Canadian Immigration Consultant (RCIC) <strong>{{owner.fullName}}</strong> (the &ldquo;RCIC&rdquo;), located at {{agency.address}}, and Client <strong>{{client.fullName}}</strong> (the &ldquo;Client&rdquo;), located at {{client.address}}.</p>
...
<table>
  ...
  <tbody>
    <tr><td>{{agreement.installment1Label}}</td><td>{{agreement.installment1Amount}}</td><td>{{agreement.installment1DueDate}}</td></tr>
    <tr><td>{{agreement.installment2Label}}</td><td>{{agreement.installment2Amount}}</td><td>{{agreement.installment2DueDate}}</td></tr>
    <tr><td>{{agreement.installment3Label}}</td><td>{{agreement.installment3Amount}}</td><td>{{agreement.installment3DueDate}}</td></tr>
    <tr><td><strong>Total</strong></td><td><strong>{{agreement.totalFees}}</strong></td><td></td></tr>
  </tbody>
</table>
...
<p><strong>RCIC</strong><br/>Name: {{owner.fullName}}<br/>Address: {{agency.address}}<br/>Tel: {{owner.phone}}<br/>E-mail: {{owner.email}}</p>
