...
test("the default lead list excludes converted leads unless a status or explicit all-status view is picked", async () => {
  ...
  assert.match(listSection, /status \? \{ status \} : includeConverted \? \{\} : \{ status: \{ not: "CONVERTED" \} \}/);
});
