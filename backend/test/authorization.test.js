...
import { caseAccessWhere, clientAccessWhere, requireRole } from "../src/middleware/authorization.js";

const auth = (role, userId = "user-a", agencyId = "agency-a") => ({ auth: { role, userId, agencyId } });
...
test("admin cannot access another agency because identity scope is membership-derived", async () => {
  const source = await readFile(new URL("../src/middleware/authMiddleware.js", import.meta.url), "utf8");
...
test("consultant client scope includes directly assigned clients and assigned cases", () => {
  const where = JSON.stringify(clientAccessWhere(auth("consultant", "consultant-1")));
...
test("client can access their own application through their portal link", () => {
  const where = JSON.stringify(caseAccessWhere(auth("client", "client-user-1")));
...
test("client cannot access another client because no foreign portal user is in scope", () => {
  const where = JSON.stringify(clientAccessWhere(auth("client", "client-user-1")));
...
test("client cannot access internal CRM role middleware", () => {
  ...
  requireRole("admin", "consultant")(auth("client"), res, () => assert.fail("client was admitted"));
...
test("frontend-provided agency_id is ignored", async () => {
  const source = await readFile(new URL("../src/utils/prismaCrud.js", import.meta.url), "utf8");
...
test("consultant cannot create another admin", () => {
  let status;
  const res = { status(value) { status = value; return this; }, json() {} };
  requireRole("admin")(auth("consultant"), res, () => assert.fail("consultant was admitted"));
  assert.equal(status, 403);
});

test("follow-ups use the internal staff picker without opening the admin user directory", async () => {
  const source = await readFile(new URL("../../frontend/src/pages/FollowUps.jsx", import.meta.url), "utf8");
...
test("consultants have personal settings without receiving admin settings controls", async () => {
  ...
  assert.match(routes, /path="\/app\/settings" element=\{<Internal /);
  assert.match(sidebar, /consultantNavigation[\s\S]*label: "Settings"/);
  assert.match(settings, /const consultantSettingsItems/);
  assert.match(settings, /isAdmin \? adminSettingsItems : consultantSettingsItems/);
});

test("consultant self-service profile updates remain user-owned and validate avatar files", async () => {
  const [routes, controller, upload, migration, authMiddleware] = await Promise.all([
    ...
    readFile(new URL("../src/middleware/profileAvatarUpload.js", import.meta.url), "utf8"),
    readFile(new URL("../prisma/migrations/20260715150000_add_user_avatar/migration.sql", import.meta.url), "utf8"),
  ...
  assert.match(controller, /id: req\.auth\.userId, agencyId: req\.auth\.agencyId/);
...
test("RLS blocks cross-agency reads and updates", async () => {
  const sql = await readFile(new URL("../prisma/migrations/20260714121000_add_auth_authorization_foundation/migration.sql", import.meta.url), "utf8");
  assert.match(sql, /ENABLE ROW LEVEL SECURITY/);
  assert.match(sql, /clients_read[\s\S]*agency_id" = current_agency_id\(\)/);
  assert.match(sql, /clients_update_staff[\s\S]*WITH CHECK \("agency_id" = current_agency_id\(\)\)/);
