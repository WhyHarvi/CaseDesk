...
/**
 * Notifies a client that a payment request now exists on their file, using
 * the agency's customized (or default) email/SMS wording and their configured
 * payment instructions. The request may still be AwaitingPaymentMethod before
 * its final QuickBooks invoice is created. Best-effort on every channel — an
 * unconfigured mailbox or Twilio line must never block the invoice itself.
 * Shared by both schedule-triggered installments and manually-created invoices.
 */
