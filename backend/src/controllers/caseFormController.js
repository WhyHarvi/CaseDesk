...
export async function monitorAssignedCaseFormRevisions({ agencyId = null, formIds = null, limit = 250 } = {}) {
  ...
  for (const form of forms) {
    ...
    if (versionStatus !== form.versionStatus) await recordFormAudit({ form: updated, event: "RevisionStatusChanged", details: `Revision status changed from ${form.versionStatus} to ${versionStatus}`, metadata: { from: form.versionStatus, to: versionStatus, availableRevision } });
  }
  return summary;
}
