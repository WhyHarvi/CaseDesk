...
export async function getAvailability(req, res) {
  ...
  const { days, settings } = await availabilityForRange({
    ...
    meetingMode,
    ignorePastCutoff: canBookPastInternalSlot(req.auth.role), ignoreLocationClosure: STAFF_ROLES_IGNORING_LOCATION_CLOSURE.has(req.auth.role),
  });
...
export async function rescheduleBookingAppointment(req, res) {
  ...
  const offeredAvailability = await availabilityForRange({ agencyId: req.auth.agencyId, assignedToId: existing.assignedToId, durationMinutes: duration, sessionBufferMinutes: effectiveBuffer, fromKey: dayKey, toKey: dayKey, excludeAppointmentId: existing.id, minNoticeOverrideMinutes: 0, locationId: meetingMode === MEETING_MODES.IN_PERSON ? selectedLocation?.id || existing.locationId || null : null, meetingMode, ignorePastCutoff: canBookPastInternalSlot(req.auth.role), ignoreLocationClosure: STAFF_ROLES_IGNORING_LOCATION_CLOSURE.has(req.auth.role) });
