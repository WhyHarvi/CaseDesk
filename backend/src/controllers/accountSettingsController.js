...
import { getAuthenticatedUser } from "../services/supabaseAuth.js";
...
export async function getAccountSecurity(req, res) {
  const [lastPasswordEvent, recentSecurity, providerUser] = await Promise.all([
    ...
    getAuthenticatedUser(req.accessToken).catch(
      () => req.authProviderUser || {},
    ),
  ]);
  const authUser = providerUser || req.authProviderUser || {};
    ...
    data: {
      account: {
        ...
        mustChangePassword: Boolean(
          req.appUser.mustChangePassword || req.membership.mustChangePassword,
        ),
      },
      authentication: {
        ...
        verifiedFactorCount: factors.filter(
          (factor) => factor.status === "verified",
        ).length,
      },
      recentSecurity: recentSecurity.map((event) => ({
        ...event,
        label: securityEventLabel(event.action),
      })),
    },
  });
}
