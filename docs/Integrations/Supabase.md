...
Frontend users authenticate with the public anon key. The backend verifies access tokens against Supabase JWKS and uses the service-role key only for administrative Auth and Storage REST calls. Client onboarding does not email Supabase's single-use GET verification URL: CaseDesk emails a signed seven-day link and confirms/sets the Supabase identity only after an explicit password-form POST, preventing mail-link scanners from consuming invitations.
...
`SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, optional `PORTAL_INVITE_SIGNING_KEY`, `SUPABASE_DOCUMENT_BUCKET`, `SUPABASE_AVATAR_BUCKET`, `SUPABASE_VOICE_TUNE_BUCKET`, `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`.
