...
- Milestone hooks: case, lifecycle, and follow-up controllers. Each queues a durable `IncentiveMilestoneEvent` row (`incentiveExpansionService.recordPendingStudyPermitMilestoneEvent`) before the immediate best-effort `creditStudyPermitMilestone` attempt, instead of that attempt being the only record a qualifying event ever happened. `updateCase` and the lifecycle "Submitted" transition queue it with `tx` inside the same transaction that writes `CaseStageHistory`, so the stage change can't commit without a durable retry record; the follow-up completion hook queues it first (no transaction is reachable through the generic CRUD helper there) before attempting to credit. The hourly incentive retry worker sweeps `PENDING` rows through `creditStudyPermitMilestone`, which is itself idempotent, so replaying an already-credited event is always safe.
- Proportional multi-collection refund allocation: `incentiveExpansionService.reverseRevenueCredits` walks every positive `IncentiveRevenueCredit` on the invoice oldest-first (not just the single oldest one), taking whatever each still has available until the refund amount is used up — an invoice paid in installments produces one credit per collection, and a refund can now span more than one of them instead of giving up with `fully_reversed` once the first is exhausted. Each reversal row is keyed by `` `${triggerRef}:${original.id}` `` so one refund event can safely create more than one row under the existing `@@unique([agencyId, triggerSource, triggerRef])` constraint, and reuses its own original credit's snapshotted period/owner/`revenueCategory` instead of re-deriving them from the current `AgencyFeeCategory`/case config, so a later category or ownership change can never block a historically valid reversal.
