...
In Nova chat, typing `/` opens a small command list. `/hide` hides both the roaming cat and the in-chat companion, while `/follow` restores them; `/stop` and `/show` are accepted aliases. These presentation-only commands run entirely in the frontend, never enter the Ollama conversation history, and persist as a browser preference across navigation and reloads.
