...
Chats can also open a pre-addressed manual email composer for an unconverted lead. That path preserves Communications email permission and connected-mailbox checks, but records the result in `LeadMessageDelivery` and lead activity rather than creating a client communication thread. Lead email currently sends immediately; client-only draft scheduling and attachments are intentionally unavailable until the lead becomes a client.

Call history keeps `All` and `Needs matching` as primary filters. Secondary filters (`Missed calls`, `Callback due`, and `Outgoing`) live under the longer `More` pill and remain URL-backed. `Callback due` is derived from tenant- and access-scoped call evidence: an inbound missed call remains due until a later successful outbound call reaches the same normalized number. The server resolves this state before pagination so counts and pages are not limited to the browser's current 25 rows.
