...
export default function ClientBillingCard({ clientId, clientName, onOpenStatement, onEntrySaved, initialEntry = null, id, highlightClassName = "" }) {
  ...
  return (
    <article id={id} className={`overflow-hidden rounded-[1.9rem] border border-white/80 bg-white/88 shadow-[0_18px_55px_rgba(15,23,42,0.08)] backdrop-blur-xl ${highlightClassName}`}>
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-2 border-t border-slate-100 px-6 py-5">
          ...
          <div className="border-t border-slate-100">
            <div className="flex items-center justify-between px-6 py-3">
              <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400">Recent activity</p>
              {data?.sourceCounts ? <p className="text-[10px] text-slate-400">{data.sourceCounts.appointments} appointment · {data.sourceCounts.caseInvoices} case invoice</p> : null}
            </div>
            {recent.length ? (
                  ...
                  return (
                    <div key={item.id} className="flex items-center gap-3 px-6 py-3.5">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2"><p className="truncate text-xs font-semibold text-slate-800">{item.type}</p><span className="truncate text-[10px] text-slate-400">{item.reference}</span></div>
                      ...
                      <div className="shrink-0 text-right">
                        ...
                        <p className="mt-0.5 text-[10px] text-slate-400">{date(item.date)}</p>
                      </div>
                    </div>
                  );
                })}
