...
function TemplateRow({ template, onSaved, onDeleted, isAdmin }) {
  const [open, setOpen] = useState(false);
  const [name, setName] = useState(template.name);
  const [caseTagsText, setCaseTagsText] = useState(tagsToText(template.caseTags));
  const [isSystemDefault, setIsSystemDefault] = useState(Boolean(template.isSystemDefault));
  ...
  async function save() {
    ...
    setError("");
    try {
      const updated = await updateScheduleTemplate(template.id, { name, installments, caseTags: textToTags(caseTagsText), isSystemDefault });
      onSaved(updated);
      setOpen(false);
    } catch (reason) {
      ...
      <button type="button" onClick={() => setOpen((v) => !v)} className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left">
        <div className="min-w-0">
          <p className="flex items-center gap-1.5 truncate text-sm font-semibold text-slate-900">
            {template.name}
            {template.isSystemDefault ? <span className="shrink-0 rounded-full bg-emerald-50 px-1.5 py-0.5 text-[10px] font-semibold text-emerald-700">System</span> : null}
          </p>
          <p className="text-xs text-slate-400">{template.caseType} · {template.installments.length} installment{template.installments.length === 1 ? "" : "s"}{template.caseTags?.length ? ` · matches: ${template.caseTags.join(", ")}` : ""}</p>
        </div>
        ...
        {open ? (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
            <div className="space-y-3 border-t border-slate-100 p-4">
              ...
              <label className="block text-[11px] font-medium text-slate-500">Matches case types tagged (comma-separated)
                <input value={caseTagsText} onChange={(event) => setCaseTagsText(event.target.value)} placeholder="e.g. express entry, pr express entry" className={`mt-1.5 ${inputClass}`} />
              </label>
              {isAdmin ? (
                <label className="flex items-center gap-2 text-xs font-medium text-slate-600">
                  <input type="checkbox" checked={isSystemDefault} onChange={(event) => setIsSystemDefault(event.target.checked)} className="h-4 w-4 rounded border-slate-300" />
                  System default for these case types
                </label>
              ) : null}
...
export default function PaymentScheduleTemplatesCard() {
  ...
  const [newTemplate, setNewTemplate] = useState({ name: "", caseType: "", caseTagsText: "", isSystemDefault: false, installments: [blankInstallment()] });
  ...
  async function submitCreate(event) {
    ...
    setCreateError("");
    try {
      const created = await createScheduleTemplate({ ...newTemplate, caseTags: textToTags(newTemplate.caseTagsText) });
      setTemplates((current) => [...(current || []), created]);
      setCreating(false);
      setNewTemplate({ name: "", caseType: "", caseTagsText: "", isSystemDefault: false, installments: [blankInstallment()] });
    } catch (reason) {
      ...
      <AnimatePresence>
        {creating ? (
          ...
          >
            ...
            <label className="block text-[11px] font-medium text-slate-500">Matches case types tagged (comma-separated)
              <input value={newTemplate.caseTagsText} onChange={(event) => setNewTemplate((c) => ({ ...c, caseTagsText: event.target.value }))} placeholder="e.g. express entry, pr express entry" className={`mt-1.5 ${inputClass}`} />
            </label>
            {isAdmin ? (
              <label className="flex items-center gap-2 text-xs font-medium text-slate-600">
                <input type="checkbox" checked={newTemplate.isSystemDefault} onChange={(event) => setNewTemplate((c) => ({ ...c, isSystemDefault: event.target.checked }))} className="h-4 w-4 rounded border-slate-300" />
                System default for these case types
              </label>
            ) : null}
