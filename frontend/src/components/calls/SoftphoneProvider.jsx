...
import { ArrowLeftRight, Loader2, Phone, PhoneCall, PhoneIncoming, PhoneOff, Search, UserRoundSearch, X } from "lucide-react";
...
export function SoftphoneProvider({ children }) {
  ...
  const [active, setActive] = useState(null); // { call, number, direction }
  ...
  const dial = useCallback(async (number) => {
    ...
    setActive({ call, number: target, direction: "OUTBOUND" });
    setMuted(false);
    call.on("disconnect", () => {
      setActive((current) => (current?.call === call ? null : current));
      setMuted(false);
    });
    call.on("cancel", () => setActive((current) => (current?.call === call ? null : current)));
    return call;
  }, [active, registered]);
