...
import AppointmentProfileOverlay from "./AppointmentProfileOverlay";
...
export default function ClientAppointmentsCard({ clientId }) {
  ...
  return <>
    <section className="overflow-hidden rounded-[2rem] border border-white/80 bg-white/88 shadow-[0_18px_55px_rgba(15,23,42,0.08)] backdrop-blur-xl">
      ...
      {loading ? <div className="flex items-center justify-center gap-2 py-12 text-sm text-slate-400"><Loader2 className="h-4 w-4 animate-spin" />Loading appointments…</div> : <div className="divide-y divide-slate-100">{appointments.length ? appointments.map((item) => <button key={item.id} type="button" onClick={() => setSelectedId(item.id)} className="grid w-full gap-3 px-5 py-4 text-left transition hover:bg-slate-50/70 sm:grid-cols-[auto_1.2fr_0.8fr_auto] sm:items-center"><span className="flex h-11 w-11 flex-col items-center justify-center rounded-2xl bg-slate-100"><span className="text-[9px] font-bold uppercase text-slate-400">{new Date(item.startsAt).toLocaleDateString("en-CA", { month: "short" })}</span><span className="text-base font-semibold leading-4 text-slate-800">{new Date(item.startsAt).getDate()}</span></span><span className="min-w-0"><span className="block truncate text-sm font-semibold text-slate-900">{item.subject}</span><span className="mt-1 block truncate text-xs text-slate-400">{item.purpose || item.description || item.sessionType?.name || "No purpose recorded"}</span></span><span className="min-w-0"><span className="block truncate text-xs font-semibold text-slate-700"><UserRound className="mr-1 inline h-3.5 w-3.5" />{item.assignedTo?.fullName || "Unassigned"}</span><span className="mt-1 block truncate text-xs text-slate-400"><Video className="mr-1 inline h-3.5 w-3.5" />{when(item.startsAt)}{item.case?.caseType ? ` · ${item.case.caseType}` : ""}</span></span><span className="flex items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${statusTone[item.status] || "bg-slate-100 text-slate-600"}`}>{item.status === "NoShow" ? "No-show" : item.status}</span>{item._count?.notes ? <span title={`${item._count.notes} notes`} className="inline-flex items-center gap-1 text-[10px] font-semibold text-violet-600"><NotebookPen className="h-3.5 w-3.5" />{item._count.notes}</span> : null}</span></button>) : <div className="px-6 py-14 text-center"><CalendarDays className="mx-auto h-6 w-6 text-slate-300" /><p className="mt-3 text-sm font-semibold text-slate-700">No {scope} appointments</p><p className="mt-1 text-sm text-slate-400">Appointments linked to this client will appear here.</p></div>}</div>}
      {meta.pages > 1 ? <footer className="flex items-center justify-between border-t border-slate-100 px-4 py-3"><span className="text-xs text-slate-400">Page {meta.page} of {meta.pages}</span><div className="flex gap-2"><button type="button" disabled={page <= 1} onClick={() => setPage((value) => value - 1)} className="flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 disabled:opacity-30"><ChevronLeft className="h-4 w-4" /></button><button type="button" disabled={page >= meta.pages} onClick={() => setPage((value) => value + 1)} className="flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 disabled:opacity-30"><ChevronRight className="h-4 w-4" /></button></div></footer> : null}
    </section>
    {selectedId ? <AppointmentProfileOverlay appointmentId={selectedId} onClose={() => setSelectedId(null)} onChanged={load} /> : null}
  </>;
}
