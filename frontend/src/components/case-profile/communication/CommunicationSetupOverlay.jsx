...
export default function CommunicationSetupOverlay({ channel, reason, detail, caseItem, role, onClose }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[480] flex items-center justify-center bg-slate-950/25 p-4 backdrop-blur-md">
      ...
      <motion.section initial={{ opacity: 0, y: 14, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="relative w-full max-w-md overflow-hidden rounded-[1.8rem] border border-white/80 bg-white shadow-[0_30px_90px_rgba(15,23,42,0.24)]">
        <div className="p-6">
          ...
          <h3 className="mt-5 text-xl font-semibold tracking-tight text-slate-950">{missingContact ? `Add the client’s ${channel === "Email" ? "email" : "phone number"}` : permissionMissing ? `${config.title} access is restricted` : `Connect ${config.title}`}</h3>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-slate-100 bg-slate-50/75 p-4">
