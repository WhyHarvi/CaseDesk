...
export function RepeatableProfileQuestionnaire({ kind, initialData = {}, saving, error, onSave }) {
  ...
  useEffect(() => {
    if (open) return;
    setEntries((initialData[config.listKey] || []).map((entry) => ({ ...blankFrom(config.fields), ...entry })));
    setComplete(initialData.complete || "No");
    setGate(config.gateKey ? initialData[config.gateKey] || "" : "");
...
export function ParentsProfileQuestionnaire({ initialData = {}, saving, error, onSave }) {
  ...
  useEffect(() => {
    if (open) return;
    setMother({ ...blankFrom(parentFields("mother")), ...(initialData.mother || {}) });
    setFather({ ...blankFrom(parentFields("father")), ...(initialData.father || {}) });
    setComplete(initialData.complete || "No");
  }, [initialData, open]);
