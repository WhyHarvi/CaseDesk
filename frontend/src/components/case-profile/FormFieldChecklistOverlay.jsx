...
export default function FormFieldChecklistOverlay({ item, caseId, onClose, onFormChanged }) {
  ...
  const sendableFields = useMemo(
    () => (checklist?.fields || []).filter((field) => field.fillableBy !== "Consultant" && field.status === "Unset"),
    [checklist],
  );
        ...
        <main className="min-h-0 flex-1 overflow-y-auto bg-slate-50/60">
          {loading ? (
            ...
            <div className="px-6 py-14 text-center">
              ...
              <p className="mt-3 text-sm font-semibold text-slate-700">Checklist not available</p>
              <p className="mx-auto mt-1 max-w-sm text-xs leading-5 text-slate-500">{checklist.reason}</p>
            </div>
          ) : (
            <>
              <div className="border-b border-slate-100 bg-white px-5 py-4">
                ...
                <div className="relative mt-1.5 max-w-sm">
                  ...
                  >
                    <option value="">Case's assigned consultant (default)</option>
                    {representatives.map((user) => (
                      <option key={user.id} value={user.id}>{user.fullName} · {user.role === "admin" ? "Admin" : "Consultant"}</option>
                    ))}
                  </select>
                  <ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                </div>
                <p className="mt-1.5 text-[11px] text-slate-400">Either an admin or a consultant can be the named representative — pick whoever is acting on this file.</p>
              </div>
