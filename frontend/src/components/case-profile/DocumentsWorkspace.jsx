...
export default function DocumentsWorkspace({ caseId, caseType, documents, assignments, saving, error, onCreateDocuments, onMarkReceived, onUpdateAssignment, onUploadMyDocument, onRefreshDocuments, onDeleteDocument, onChangeDocumentStatus }) {
  ...
  const folders = [
    ...
    { id: "shared", title: "Shared Library", subtitle: "Agency templates and reusable resources", count: sharedDocuments.length },
          ...
          {openFolder ? (
            <motion.section key={openFolder} initial={{ opacity: 0, y: 8, height: 0 }} animate={{ opacity: 1, y: 0, height: "auto" }} exit={{ opacity: 0, y: -5, height: 0 }} transition={{ duration: 0.24, ease: [0.22, 1, 0.36, 1] }} className="mt-4 overflow-hidden rounded-[1.25rem] border border-white/80 bg-white shadow-sm">
              ...
              <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 bg-slate-50/45 px-4 py-2.5">
                ...
                {["client", "finalized", "unassigned"].includes(openFolder) ? <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="h-9 rounded-xl border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-600 outline-none focus:border-sky-300"><option value="all">All statuses</option>{documentStatusOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select> : null}
                {openFolder === "shared" ? <select value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value)} className="h-9 rounded-xl border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-600 outline-none focus:border-sky-300"><option value="all">All categories</option>{availableSharedCategories.map((category) => <option key={category}>{category}</option>)}</select> : null}
                <select value={sortOrder} onChange={(event) => setSortOrder(event.target.value)} className="h-9 rounded-xl border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-600 outline-none focus:border-sky-300"><option value="newest">Newest first</option><option value="oldest">Oldest first</option><option value="name">Name A–Z</option><option value="status">Status</option></select>
