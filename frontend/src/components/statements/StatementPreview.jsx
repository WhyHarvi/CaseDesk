    ...
    <article className="statement-print-root statement-print-card overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-[0_18px_55px_rgba(15,23,42,0.08)]">
      <div className="p-6 sm:p-8">
        ...
        <div className="grid gap-6 border-b border-slate-200 py-6 sm:grid-cols-2">
          ...
          <dl className="grid grid-cols-2 gap-4 text-xs"><div><dt className="font-bold uppercase tracking-wide text-slate-400">Generated</dt><dd className="mt-1 text-slate-700">{formatDate(statement, statement.generatedAt)}</dd></div><div><dt className="font-bold uppercase tracking-wide text-slate-400">Currency</dt><dd className="mt-1 text-slate-700">{statement.currency}</dd></div><div><dt className="font-bold uppercase tracking-wide text-slate-400">Period</dt><dd className="mt-1 text-slate-700">{period}</dd></div><div><dt className="font-bold uppercase tracking-wide text-slate-400">Case</dt><dd className="mt-1 text-slate-700">{statement.case?.caseType || "All Cases"}</dd></div></dl>
        </div>
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[720px] border-collapse text-left text-xs"><thead><tr className="border-y border-slate-200 bg-slate-50 text-[9px] uppercase tracking-wide text-slate-400"><th className="px-2 py-3">Date</th><th className="px-2 py-3">Reference</th><th className="px-2 py-3">Type / Description</th><th className="px-2 py-3">Case</th><th className="px-2 py-3 text-right">Charge</th><th className="px-2 py-3 text-right">Payment / Credit</th><th className="px-2 py-3 text-right">Balance</th></tr></thead><tbody>{statement.transactions.length ? statement.transactions.map((item) => <tr key={item.id} className="border-b border-slate-100 align-top"><td className="whitespace-nowrap px-2 py-3 text-slate-500">{formatDate(statement, item.date)}</td><td className="px-2 py-3 font-medium text-slate-700">{item.reference}</td><td className="px-2 py-3"><span className="font-semibold text-slate-800">{item.type}</span><span className="mt-0.5 block text-slate-400">{item.description}</span></td><td className="px-2 py-3 text-slate-500">{item.caseReference || ""}</td><td className="px-2 py-3 text-right text-slate-700">{item.charge ? formatMoney(statement, item.charge) : ""}</td><td className="px-2 py-3 text-right text-slate-700">{item.paymentOrCredit ? formatMoney(statement, item.paymentOrCredit) : ""}</td><td className="px-2 py-3 text-right font-semibold text-slate-900">{formatMoney(statement, item.runningBalance)}</td></tr>) : <tr><td colSpan="7" className="px-4 py-10 text-center text-sm text-slate-400">No transactions for the selected period.</td></tr>}</tbody></table></div>
