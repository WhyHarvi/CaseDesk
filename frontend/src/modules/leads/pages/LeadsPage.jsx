            ...
            <table className="w-full min-w-[960px] border-collapse text-left">
              ...
              <tbody>{groupedLeads.map((group) => (
                <Fragment key={group.key ?? "all"}>
                  ...
                  {group.items.map((lead) => { const due = formatDueDate(lead.nextActionAt); const blockedOnConversion = lead.status === "OPEN" && lead.stage === "READY_TO_CONVERT" && !(RETAINER_READY_VALUES.includes(lead.retainerStatus) && PAYMENT_READY_VALUES.includes(lead.initialPaymentStatus)); const latestNote = lead.activities?.[0]?.description; return <tr key={lead.id} role="button" tabIndex={0} onClick={() => setSelectedLead(lead)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelectedLead(lead); } }} className="cursor-pointer border-b border-slate-100 text-sm outline-none transition last:border-0 hover:bg-brand-50/45 focus:bg-brand-50/70">
                    ...
                    <td className="max-w-[170px] px-3 py-3 text-xs text-slate-500"><p className="line-clamp-2">{latestNote || "No notes"}</p></td>
                  </tr>; })}
                </Fragment>
              ))}</tbody>
            </table>
