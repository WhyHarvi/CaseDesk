        ...
        {loading ? <ListSkeleton /> : !leads.length ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[960px] border-collapse text-left">
              <thead><tr className="border-b border-slate-200 bg-slate-50/70 text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-500">{canBulkSelect ? <th className="w-9 px-3 py-2.5"><SelectAllCheckbox checked={selectedIds.size > 0 && selectedIds.size === leads.length} indeterminate={selectedIds.size > 0 && selectedIds.size < leads.length} onChange={toggleSelectAll} /></th> : null}<th className="px-3 py-2.5">Lead</th><th className="px-2.5 py-2.5">Source and interest</th><th className="px-2.5 py-2.5">Progress</th><th className="px-2.5 py-2.5">Owner</th><th className="px-2.5 py-2.5">Next action</th><th className="px-3 py-2.5">Notes</th></tr></thead>
              <tbody>{groupedLeads.map((group) => (
                <Fragment key={group.key ?? "all"}>
                  {group.label ? <tr><td colSpan={canBulkSelect ? 7 : 6} className="bg-slate-100/80 px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.1em] text-slate-500">{group.label} · {group.items.length} lead{group.items.length === 1 ? "" : "s"}</td></tr> : null}
                  {group.items.map((lead) => { const due = formatDueDate(lead.nextActionAt); const blockedOnConversion = lead.status === "OPEN" && lead.stage === "READY_TO_CONVERT" && !(RETAINER_READY_VALUES.includes(lead.retainerStatus) && PAYMENT_READY_VALUES.includes(lead.initialPaymentStatus)); const latestNote = lead.activities?.[0]?.description;
                    const displayedNote = latestNote || lead.initialMessage;
                    const isInitialInquiry = !latestNote && Boolean(lead.initialMessage); return <tr key={lead.id} role="button" tabIndex={0} onClick={() => setSelectedLead(lead)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelectedLead(lead); } }} className="cursor-pointer border-b border-slate-100 text-sm outline-none transition last:border-0 hover:bg-brand-50/45 focus:bg-brand-50/70">
                    {canBulkSelect ? <td className="px-3 py-3" onClick={(event) => event.stopPropagation()}><input type="checkbox" checked={selectedIds.has(lead.id)} onChange={() => toggleSelected(lead.id)} className="h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-200" aria-label={`Select ${leadName(lead)}`} /></td> : null}
                    <td className="px-3 py-3"><div className="flex items-center gap-2.5"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-900 text-xs font-semibold text-white">{initials(lead)}</div><div className="min-w-0"><p className="truncate font-semibold text-slate-900">{leadName(lead)}</p><p className="mt-0.5 truncate text-xs text-slate-400">{lead.leadNumber} · {lead.phone}</p></div>{lead.phone ? <button type="button" disabled={softphoneStatus !== "ready"} onClick={(event) => { event.stopPropagation(); startCall(lead); }} title={softphoneStatus !== "ready" ? "Connect Twilio calling in Settings to call this lead" : `Call ${leadName(lead)}`} className="ml-auto flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-500 shadow-sm transition hover:border-sky-300 hover:bg-sky-50 hover:text-sky-600 active:scale-95 disabled:cursor-not-allowed disabled:opacity-40"><Phone className="h-3.5 w-3.5" /></button> : null}</div></td>
                    <td className="px-2.5 py-3"><p className="max-w-[150px] truncate font-medium text-slate-700">{lead.originalSource?.name || "Unknown source"}</p><p className="mt-0.5 max-w-[150px] truncate text-xs text-slate-400">{lead.immigrationInterest || "Interest not specified"}</p></td>
                    <td className="px-2.5 py-3"><div className="flex flex-wrap items-center gap-1.5"><span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold ring-1 ring-inset ${statusTone[lead.status] || statusTone.ARCHIVED}`}>{humanize(lead.status)}</span><span className="text-xs font-medium text-slate-600">{humanize(lead.stage)}</span>{blockedOnConversion ? <span className="inline-flex items-center rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-semibold text-amber-700 ring-1 ring-inset ring-amber-100" title="Ready to Convert, but retainer or payment isn't confirmed yet">Blocked</span> : null}</div><p className="mt-1.5 text-xs text-slate-400">{humanize(lead.temperature)} · {humanize(lead.priority)}</p></td>
                    <td className="max-w-[130px] truncate px-2.5 py-3 text-slate-700">{lead.owner?.fullName || "Unassigned"}</td>
                    <td className="px-2.5 py-3"><p className="max-w-[150px] truncate font-medium text-slate-700">{lead.nextActionDescription || "No next action"}</p><p className={`mt-0.5 text-xs ${due.overdue && lead.status === "OPEN" ? "font-semibold text-rose-600" : "text-slate-400"}`}>{due.overdue && lead.status === "OPEN" ? "Overdue · " : ""}{due.label}</p></td>
                    <td className="max-w-[170px] px-3 py-3 text-xs text-slate-500">{isInitialInquiry ? <p className="line-clamp-2 text-slate-400"><span className="font-semibold uppercase tracking-[0.06em]">Inquiry: </span><span className="italic">{displayedNote}</span></p> : <p className="line-clamp-2">{displayedNote || "No notes"}</p>}</td>
                  </tr>; })}
                </Fragment>
              ))}</tbody>
            </table>
