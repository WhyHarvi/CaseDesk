import { Check, Clipboard, Code2, ExternalLink, FileSpreadsheet, Inbox, KeyRound, Link2, Plus, RefreshCw, ShieldCheck, Upload, Waypoints } from "lucide-react";
...
const tabs = [{ id: "forms", label: "Public forms", icon: Link2, roles: ["admin", "consultant", "frontdesk"] }, { id: "website", label: "Connections", icon: Code2, roles: ["admin"] }, { id: "imports", label: "CSV imports", icon: FileSpreadsheet, roles: ["admin", "consultant"] }, { id: "events", label: "Incoming events", icon: Inbox, roles: ["admin"] }];
...
export default function LeadIntakePage() {
  ...
  return <section className="space-y-5 pb-8">
    ...
    {tab === "forms" ? <div className="space-y-4">
      <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-5 py-4"><div><p className="text-sm font-semibold text-slate-900">{activeCount} active public {activeCount === 1 ? "form" : "forms"}</p><p className="mt-1 text-xs text-slate-500">Each link is secure, stable, and ready for a website button or QR code.</p></div>{role !== "frontdesk" ? <button onClick={() => setFormOpen(true)} className="inline-flex h-10 items-center gap-2 rounded-full bg-slate-900 px-4 text-sm font-semibold text-white"><Plus className="h-4 w-4" />New form</button> : null}</div>
