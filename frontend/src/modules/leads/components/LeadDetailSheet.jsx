...
export default function LeadDetailSheet({ lead: initialLead, staff = [], onClose, onChanged = () => {} }) {
  ...
  useEffect(() => {
    const close = (event) => event.key === "Escape" && !bookingOpen && !commercialStatusOpen && !conversionOpen && !activeAction && !closingFollowUp && onClose();
    window.addEventListener("keydown", close);
    return () => window.removeEventListener("keydown", close);
  }, [bookingOpen, commercialStatusOpen, conversionOpen, activeAction, closingFollowUp, onClose]);
              ...
              {tab === "work" ? (
                <div className="space-y-5">
                  ...
                  <section>
                    ...
                    <div className="overflow-hidden rounded-2xl border border-slate-200/70 bg-white">{consultations.length ? consultations.map((item, index) => <div key={item.id} className={[
                      "px-5 py-4",
