...
const adminSettingsItems = [
  ...
  },
  {
    ...
    title: "System Email",
    subtitle:
      "Manage the shared sender used for appointments, billing, and automated notices.",
  },
  {
    id: "agency-phone",
    label: "Phone & SMS",
...
function PersonalProfilePanel({ user, agency }) {
  ...
  async function saveProfile(event) {
    ...
    try {
      ...
      setError(
        reason.response?.data?.message || "Your profile could not be updated.",
      );
    } finally {
      setSaving(false);
    }
  ...
  const initials = (profile.fullName || "Consultant")
    .split(" ")
    .filter(Boolean)
    .map((part) => part[0])
    .join("")
      ...
      <header className="grid gap-8 border-b border-slate-200 pb-10 sm:grid-cols-[220px_minmax(0,1fr)] sm:gap-10 lg:grid-cols-[290px_minmax(0,1fr)]">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-3">
            <h2 className="truncate text-xl font-normal text-slate-950">
              {profile.fullName || "Consultant"}
            </h2>
            <button
              type="button"
              disabled={loading}
          ...
          <div className="mt-7 text-sm leading-6">
            <p className="font-semibold text-slate-950">
              {profile.jobTitle || "Immigration Consultant"}
            </p>
            <p className="text-slate-600">
              {agency?.name || "CaseDesk agency"}
            </p>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {profile.specializations?.length ? (
              ...
              <span className="text-sm text-slate-400">
                Add specialties to complete your profile.
              </span>
            )}
          </div>
        </div>
        ...
        {[
          ...
          return (
            ...
            >
              ...
              <div
                className={`text-[14px] font-medium sm:text-[15px] ${row.value ? "text-slate-950" : "text-slate-400"}`}
              >
                {row.value || "Not added"}
              </div>
            </div>
        ...
        ? createPortal(
            ...
            >
              <form
                ...
                className="flex max-h-[94dvh] w-full max-w-[520px] flex-col overflow-hidden rounded-t-[24px] bg-white shadow-[0_30px_100px_rgba(15,23,42,0.35)] sm:max-h-[88vh] sm:rounded-[20px]"
              >
                <div className="grid h-14 shrink-0 grid-cols-[1fr_auto_1fr] items-center border-b border-slate-200 px-5">
                  ...
                  <button
                    disabled={saving}
                    className="justify-self-end text-sm font-semibold text-sky-600 disabled:opacity-50"
                  >
                    {saving ? "Saving…" : "Done"}
                  </button>
                </div>
                ...
                <div className="min-h-0 overflow-y-auto overscroll-contain">
                  <div className="flex items-center gap-4 bg-slate-50 px-6 py-5">
                    <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-full bg-slate-200 font-semibold text-slate-700">
                      {displayedAvatar ? (
                        <img
                          src={displayedAvatar}
                          alt="Profile preview"
                    ...
                    <div className="min-w-0">
                      ...
                      <label className="cursor-pointer text-sm font-semibold text-sky-600">
                  ...
                  <div className="space-y-5 p-6">
                    ...
                    <label className="block text-sm font-semibold text-slate-800">
                      ...
                      <textarea
                        ...
                        placeholder="Express Entry, Work Permits, Family Sponsorship"
                        className="mt-2 w-full resize-none rounded-lg border border-slate-300 px-3 py-3 text-sm font-normal outline-none focus:border-slate-500"
                      />
...
function FrontdeskProfilePanel({ user, agency }) {
  const initials = (user?.fullName || "Front Desk")
    .split(" ")
    .filter(Boolean)
    .map((part) => part[0])
    .join("")
  ...
  return (
    <div className="mx-auto max-w-[935px]">
      ...
      <p className="mt-4 rounded-lg border border-slate-200 px-4 py-3 text-xs leading-5 text-slate-500">
        Your profile details are managed by a workspace administrator. Contact
        them to update your name, contact information, or job title.
      </p>
    </div>
  );
}
...
export default function Settings() {
  ...
  const [activeSection, setActiveSection] = useState(() =>
    settingsItems.some((item) => item.id === requestedSection)
      ? requestedSection
      : defaultSection,
  );
    ...
    <section className="flex h-full min-h-0 w-full flex-col overflow-hidden bg-transparent">
      <div className="flex-shrink-0 px-6 pb-5 pt-6">
        ...
        <p className="mt-2 max-w-2xl text-[15px] leading-6 text-slate-500">
          {isAdmin
            ? "Manage your agency, team, workflow, and system preferences."
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  {!isAdmin && activeSection === "personal-profile" ? (
                    ...
                    <TeamMembers />
                  ) : isAdmin && activeSection === "portal-access" ? (
