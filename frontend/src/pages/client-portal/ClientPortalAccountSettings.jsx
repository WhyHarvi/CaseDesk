...
import PortalNotificationsSettingsPanel from "../../components/client-portal/PortalNotificationsSettingsPanel";
    ...
    <div className="space-y-4">
      ...
      <section className="rounded-[28px] border border-white/70 bg-white/80 p-4 shadow-[0_16px_45px_rgba(28,45,74,0.08)] backdrop-blur-2xl sm:p-6">
        <div className="mb-6 inline-flex rounded-full bg-slate-100 p-1">
          ...
          <button type="button" onClick={() => setSection("notifications")} className={`inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-[12px] font-semibold transition ${section === "notifications" ? "bg-white text-slate-950 shadow-sm" : "text-slate-500"}`}><Bell className="h-3.5 w-3.5" /> Notifications</button>
        ...
        {section === "security" ? <SecuritySettingsPanel compact /> : section === "notifications" ? <PortalNotificationsSettingsPanel /> : <ActivityLogsSettingsPanel compact />}
      </section>
    </div>
  );
}
