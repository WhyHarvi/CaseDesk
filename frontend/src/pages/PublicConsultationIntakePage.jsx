...
import { preConsultationQuestion } from "../components/appointments/preConsultationQuestions";
...
function YesNo({ name, value, onChange, label }) {
  return <fieldset className="sm:col-span-2"><legend className={labelClass}>{preConsultationQuestion(label)}</legend><div className="mt-2 flex gap-2">{[["true", "Yes"], ["false", "No"]].map(([option, text]) => {
    const selected = String(value) === option;
    return <button key={option} type="button" name={name} value={option} aria-pressed={selected} onClick={() => onChange(option)} className={`h-11 min-w-20 touch-manipulation select-none rounded-full border px-4 text-sm font-semibold outline-none transition-colors focus-visible:ring-4 focus-visible:ring-sky-100 ${selected ? "border-slate-950 bg-slate-950 text-white" : "border-slate-200 bg-white text-slate-600 hover:border-slate-300 hover:bg-slate-50"}`}>{text}</button>;
  })}</div></fieldset>;
}

function Field({ label, name, type = "text", value, onChange, required = false, placeholder = "", children }) {
  return <label className={labelClass}>{preConsultationQuestion(label)}{children || <input className={inputClass} name={name} type={type} value={value || ""} onChange={(event) => onChange(event.target.value)} required={required} placeholder={placeholder} />}</label>;
}

function SelectField({ label, name, value, onChange, options, required = false }) {
  return <label className={labelClass}>{preConsultationQuestion(label)}<select className={inputClass} name={name} value={value || ""} onChange={(event) => onChange(event.target.value)} required={required}><option value="">Select</option>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select></label>;
}
...
export default function PublicConsultationIntakePage() {
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-7 sm:py-10"><div className="mx-auto max-w-3xl">
    ...
    <form onSubmit={step === steps.length - 1 ? submit : next} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
      ...
      {step === 3 ? <div className="space-y-5"><div><h2 className="text-lg font-semibold text-slate-950">Deadlines & final details</h2><p className="mt-1 text-xs leading-5 text-slate-500">Flag anything that the consultant should review before your appointment.</p></div><div className={sectionClass}><div className="grid gap-4 sm:grid-cols-2"><YesNo name="hasDeadline" value={values.hasDeadline} onChange={set("hasDeadline")} label="Do you have an immigration deadline we should know about?" />{yes("hasDeadline") ? <><Field label="Deadline type" value={values.deadlineType} onChange={set("deadlineType")} /><Field label="Deadline date" type="date" value={values.deadlineDate} onChange={set("deadlineDate")} /><TextAreaField label="Deadline details" value={values.deadlineDetails} onChange={set("deadlineDetails")} /></> : null}<YesNo name="hasActiveIssue" value={values.hasActiveIssue} onChange={set("hasActiveIssue")} label="Do you have a refusal, removal order, loss of status, or another urgent immigration issue?" />{yes("hasActiveIssue") ? <TextAreaField label="Urgent issue details" value={values.activeIssueDetails} onChange={set("activeIssueDetails")} rows={4} /> : null}<TextAreaField label="Anything else the consultant should know?" value={values.additionalInformation} onChange={set("additionalInformation")} rows={5} /></div></div>
