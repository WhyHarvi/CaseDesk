...
export default function ManageBookingPage() {
  ...
  useEffect(() => {
    getManagedBooking(manageToken)
      .then((data) => {
        ...
        setLocationId(
          data.locationId ||
            (data.locations?.length === 1 ? data.locations[0].id : ""),
        );
      })
      .catch((reason) =>
        setLoadError(
  ...
  const fetchAvailability = useCallback(
    (range) =>
      getManagedAvailability(manageToken, {
        ...range,
        meetingMode,
        locationId:
  ...
  async function confirmCancel() {
    ...
    setError("");
    try {
      const updated = await cancelManagedBooking(
        manageToken,
        cancelReason,
        seriesScope,
      );
  ...
  async function confirmReschedule() {
    ...
    try {
      const updated = await rescheduleManagedBooking(manageToken, {
        ...
        locationId:
          meetingMode === "InPerson"
            ? locationId || booking.locations?.[0]?.id
            : undefined,
        scope: seriesScope,
      });
      setBooking(updated);
      ...
      setLocationId(
        updated.locationId ||
          (updated.locations?.length === 1 ? updated.locations[0].id : ""),
      );
      setMode("view");
      setSlot(null);
      setNotice(
        "Your appointment has been moved. A new confirmation is on its way.",
      );
    } catch (reason) {
      setError(
        reason.response?.data?.message || "The appointment could not be moved.",
      );
    } finally {
      setBusy(false);
    }
  ...
  async function confirmAttendance() {
    ...
    setError("");
    try {
      const updated = await confirmManagedBooking(manageToken);
      setBooking(updated);
      setNotice("Attendance confirmed. We’ll see you there.");
    } catch (reason) {
      setError(
        reason.response?.data?.message || "Attendance could not be confirmed.",
      );
    } finally {
      setBusy(false);
    }
  ...
  async function completePreparation() {
    ...
    setError("");
    try {
      const updated = await completeManagedPreparation(manageToken);
      setBooking(updated);
      setNotice("Preparation marked complete.");
    } catch (reason) {
      setError(
        reason.response?.data?.message || "Preparation could not be updated.",
      );
    } finally {
      setBusy(false);
    }
        ...
        {mode === "reschedule" ? (
          ...
          >
            ...
            <textarea
              ...
              className="mt-4 w-full resize-none rounded-2xl border border-slate-200 bg-white px-3.5 py-3 text-base outline-none transition focus:border-rose-300 sm:text-sm"
            />
            {booking.isRecurring ? (
            ...
            <div className="mt-6 space-y-2">
              <button
                ...
                disabled={busy}
                className="flex h-12 w-full items-center justify-center gap-2 rounded-full bg-rose-600 text-sm font-semibold text-white transition hover:bg-rose-500 disabled:opacity-60"
              >
                {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : null}{" "}
                Yes, cancel it
              </button>
              <button
                ...
                disabled={busy}
                className="h-11 w-full rounded-full text-sm font-semibold text-slate-600 transition hover:bg-slate-100"
              >
                Keep my appointment
              </button>
            </div>
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <span
                className={`flex h-11 w-11 items-center justify-center rounded-2xl ${!active ? "bg-slate-100 text-slate-400" : "bg-sky-50 text-sky-600"}`}
              >
              ...
              <div>
                ...
                <p
                  className={`text-sm ${cancelled ? "font-medium text-rose-600" : booking.status === "NoShow" ? "font-medium text-amber-700" : "text-slate-500"}`}
                >
                  {statusLabel}
                </p>
              </div>
            ...
            <div className="mt-5 space-y-3 rounded-2xl bg-slate-50 px-4 py-4 text-sm text-slate-700">
              ...
              <p className="flex items-center gap-2.5">
                ...
                {booking.sessionType?.durationMinutes ||
                  Math.round(
                    (new Date(booking.endsAt) - new Date(booking.startsAt)) /
                      60000,
                  )}{" "}
                minutes · {booking.timezone}
              </p>
              {booking.meetingMode === "Phone" ? (
                <p className="flex items-start gap-2.5">
                  <Phone className="mt-0.5 h-4 w-4 shrink-0 text-slate-400" />
                  <span>
                    Phone call — {booking.agencyName} will call you
                    {booking.meetingPhoneNumber
                      ? ` from ${booking.meetingPhoneNumber}`
                      : ""}
                    .
                  </span>
                </p>
                ...
                <p className="flex items-start gap-2.5">
                  ...
                  <span>
                    ...
                    {booking.locationMapsUrl ? (
                      <a
                        href={booking.locationMapsUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="mt-1 block text-xs font-semibold text-sky-600 hover:text-sky-700"
                      >
                        Open in Maps ↗
                      </a>
                    ) : null}
            ...
            (booking.preparationInstructions ||
              booking.preparationChecklist?.length ||
              booking.parkingInstructions) ? (
              <div className="mt-4 rounded-2xl border border-amber-100 bg-amber-50/60 px-4 py-4 text-sm text-slate-700">
                ...
                {booking.preparationChecklist?.length ? (
                  <ul className="mt-2 space-y-1">
                    {booking.preparationChecklist.map((item) => (
                ...
                {booking.parkingInstructions &&
                booking.meetingMode === "InPerson" ? (
                  <p className="mt-2 border-t border-amber-100 pt-2 text-xs text-slate-500">
                    Arrival: {booking.parkingInstructions}
                  </p>
                ) : null}
                {!booking.preparationCompletedAt ? (
                  <button
                    type="button"
                    disabled={busy}
                    onClick={completePreparation}
                    className="mt-3 rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-sm disabled:opacity-50"
                  >
                    I’m prepared
                  </button>
                ) : (
            ...
            {active && !past && booking.canConfirm ? (
              <button
                type="button"
                disabled={busy}
                onClick={confirmAttendance}
                className="mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-full bg-emerald-600 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:opacity-50"
              >
                <CheckCircle2 className="h-4 w-4" /> Confirm I’ll attend
              </button>
            ) : null}
            ...
            {active && !past && booking.meetingUrl ? (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-full bg-sky-600 text-sm font-semibold text-white transition hover:bg-sky-700"
              >
                <Video className="h-4 w-4" /> Join{" "}
                {booking.meetingMode === "Zoom" ? "Zoom" : "Jitsi"} video call
              </a>
            ) : null}

            {active && !past && (booking.canReschedule || booking.canCancel) ? (
              <div
                className={`mt-3 grid gap-2 ${booking.canReschedule && booking.canCancel ? "grid-cols-2" : "grid-cols-1"}`}
              >
                {booking.canReschedule ? (
                  <button
                    ...
                    onClick={() => {
                      setMode("reschedule");
                      setNotice("");
                      setMeetingMode(booking.meetingMode || "InPerson");
                      setLocationId(
                        booking.locationId ||
                          (booking.locations?.length === 1
                            ? booking.locations[0].id
                            : ""),
                      );
                    }}
                    className="h-12 rounded-full border border-slate-200 bg-white text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50"
                  >
                    Reschedule
                  </button>
                ) : null}
                {booking.canCancel ? (
                  <button
                    ...
                    className="h-12 rounded-full border border-rose-200 bg-rose-50 text-sm font-semibold text-rose-700 transition hover:bg-rose-100"
                  >
                    Cancel
                  </button>
                ) : null}
            ...
            {active && !past && !booking.canReschedule && !booking.canCancel ? (
              <p className="mt-4 text-center text-xs text-slate-500">
                This appointment is inside the office change cutoff. Contact{" "}
                {booking.agencyName} if you need help.
              </p>
            ) : null}

            {cancelled ? (
              <p className="mt-4 text-center text-sm text-slate-500">
                Need a new time? Ask {booking.agencyName} for their booking
                link.
              </p>
            ) : null}
          </motion.div>
